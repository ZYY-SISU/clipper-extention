...
export const DEFAULT_TEMPLATES: TemplateConfig[] = [
  ...
  },
  {
    ...
    请分析文档，提取关键信息并返回严格的 JSON 格式。

    【重要原则】
    1. **去重**：绝对不要输出重复的内容！检查 keyPoints 和 relatedLinks，确保没有重复项。
    2. **精简**：如果网页包含大量导航栏或广告干扰信息，请忽略它们，只关注正文。
    3. **数量限制**：
       - keyPoints 最多提取 5 条。
       - relatedLinks 最多提取 3 条。
       - tags 最多提取 5 个。
    ...
    {
      "title": H1 或显著标题,
      "description": "用 1~2 句说明该文档/接口能做什么、适用前提、关键限制（若有 HTTP 路径/方法请标注）",
      "category": "判定类型（如 API、SDK、CLI、组件、库、教程、指南、架构）",
      "mainSections": [
        // 提取 3-10 个主要章节标题
        "章节标题1", "章节标题2"
      ],
      "parameters": [
        // 提取参数列表 (最多 10 个)
        { "name": "参数名", "type": "类型", "desc": "简短说明", "required": true/false }
      ],
      "returns": "返回值描述",
      "examples": [
        // 提取代码示例 (最多 3 个)
